...
public class CountryResource {
    ...
    @Consumes(APPLICATION_FORM_URLENCODED)
    @Produces(APPLICATION_JSON)
    public Response updateCountry(@FormParam("capital") String capital, @FormParam("name") String name) {
        Country country = new Country(capital, name);
    ...
    public Response get(@PathParam("name") String name) {
        ...
        try {
            ...
            logger.error("Country name '{}' not found", name, e);
            return Response.status(Status.NOT_FOUND).build();
        } catch (NoResultException e) {
            logger.error("Country with name '{}' not found", name, e);
            return Response.status(Status.NOT_FOUND).build();
        }
    }
